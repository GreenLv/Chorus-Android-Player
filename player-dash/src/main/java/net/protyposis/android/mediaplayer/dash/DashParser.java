...
public class DashParser {
     ...
     * @param transportClient the transport client instance to use for the request
     * @return a MPD object
     * @throws android.os.NetworkOnMainThreadException if executed on the main thread
     */
    public MPD parse(UriSource source, TransportClient transportClient) throws DashParserException {
        MPD mpd = null;

        Uri uri = source.getUri();
        TransportRequest request = TransportRequest.get(URI.create(uri.toString()))
        ...
        try {
            ...
            if (!response.isSuccessful()) {
                throw new IOException("MPD request failed with HTTP " + response.statusCode());
            }
            ...
            serverDate = parseHttpDate(response.firstHeader("Date"));

            // Parse the MPD file
            mpd = parse(new ByteArrayInputStream(response.body()), baseUrl);
        } catch (IOException e) {
            Log.e(TAG, "error downloading the MPD (" + e.getClass().getSimpleName() + ")");
            throw new DashParserException("error downloading the MPD", e);
        } catch (XmlPullParserException e) {
            Log.e(TAG, "error parsing the MPD (" + e.getClass().getSimpleName() + ")");
    ...
    private Date parseHttpDate(String value) {
        ...
        for (String pattern : patterns) {
            try {
                ...
                // Try the next valid HTTP-date form.
            }
        }
        Log.w(TAG, "server returned an invalid Date header");
        return null;
    }
            ...
            if(type == XmlPullParser.START_TAG) {
                ...
                if(tagName.equals("SegmentTemplate")) {
                    ...
                    try {
                        ...
                        Log.e(TAG, "error reading representation ("
        ...
        while((type = parser.next()) >= 0) {
            ...
            if(type == XmlPullParser.START_TAG) {
                if (tagName.equals("Initialization")) {
                    ...
                    Log.d(TAG, "initialization segment parsed");
                    ...
                    baseUrl = extendUrl(baseUrl, parser.nextText());
                    Log.d(TAG, "representation BaseURL updated");
     ...
     *
     * DASH specification: template-based Segment URL construction, identifier table.
     */
