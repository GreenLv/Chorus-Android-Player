...
Supply the optional default transiently as `-PchorusMpdUrl=https://example.invalid/path/manifest.mpd` or in the user's `~/.gradle/gradle.properties`. The value is embedded in that local APK, so never use a credential-bearing URL and never distribute the generated APK. No tracked Android resource or project property file stores an endpoint.
