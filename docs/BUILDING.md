...
The `stub` product flavor names the public, no-private-dependency build. It currently wires the HTTPS compatibility provider so generic MPDs can be exercised; it does not expose HTTP/3, multipath, client-QoE, or server-QoE capabilities. There is no XQUIC flavor until an authorized provider is available.
